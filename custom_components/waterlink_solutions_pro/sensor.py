...
async def async_setup_entry(hass, entry, async_add_entities):
    ...
    for site_id, coordinator in coordinators.items():
        site_data = coordinator.data
        latest_test = max((d.get("_test_time") for d in site_data.values() if d.get("_test_time")), default=None)

        for factor, data in site_data.items():
            entities.append(WaterLinkSensor(coordinator, factor, data))

        if latest_test:
            entities.append(LastTestTimeSensor(site_id, latest_test, next(iter(site_data.values()), {})))
...
class WaterLinkSensor(SensorEntity):
    ...
    def __init__(self, coordinator, factor, measurement):
        ...
        self._attr_unique_id = f"waterlink_{coordinator.api._site_id}_{factor.lower().replace(' ', '_')}"
        self._factor = factor
